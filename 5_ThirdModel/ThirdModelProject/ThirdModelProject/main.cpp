...
int main() {
	...
	while (ser.isOpen()) {
		...
		printf("b%d ", read_end);
		if (read_end) {
			NEXT_STATE[previous_angle][previous_speed][present_speed].addData(max_angle);
			...
			if (cnt_loop) {
				...
				NEXT_STATE[previous_angle][previous_speed][present_speed].addData(temp_gaussian + present_angle);
			}

		}
		printf("(%4d, %4d %4d) : %4d\n", previous_angle, previous_speed, present_speed, present_angle);
		...
		for (i = 0; i < max_speed; i++) {
			...
			temp_gauss = NEXT_STATE[present_angle][present_speed][i].retFinal(temp_gauss);
	...
	for (i = 0; i < max_angle; i++) {
		for (j = 0; j < max_speed; j++) {
			for (k = 0; k < max_speed; k++) {
				fscanf(fp, " %d %d", &NEXT_STATE[i][j][k].cnt, &NEXT_STATE[i][j][k].sum);
			}			
		}
	}
...
void write_file() {
	...
	for (i = 0; i < max_angle; i++) {
		for (j = 0; j < max_speed; j++) {
			for (k = 0; k < max_speed; k++) {
				if (NEXT_STATE[i][j][k].cnt == 0)
					cnt++;
				fprintf(fp, " %d %d", int(NEXT_STATE[i][j][k].cnt), int(NEXT_STATE[i][j][k].sum));
			}			
			fprintf(fp, "\n");
		}
		fprintf(fp, "\n");
