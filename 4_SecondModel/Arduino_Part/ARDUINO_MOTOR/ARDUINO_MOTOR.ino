...
void check_signal() {
  int beat = 1, i = 0;
  for(i = 0; i < 8; i++) {
    digitalWrite(i + 5, signal_received & beat);
    beat = beat << 1;
  }
}
...
/*  digitalWrite(IN3, tmp_speed > 0 ? HIGH : LOW);
  digitalWrite(IN4, tmp_speed > 0 ? LOW : HIGH);*/
...
void check_signal() {
  int beat = 1, i = 0;
  int ib[8] = {2, 3, 4, 11, 12, A1, A2, A3};
  for(i = 0; i < 8; i++) {
    digitalWrite(ib[i], signal_received & beat);
    beat = beat << 1;
  }
}
